var SpecTestsLoader = new (function() {
    ...
    this.loadIconPath = "./assets/images/test.png";
    this.loadedIconPath = "./assets/images/done.png";
    ...
    this.insertLoadIcon = function($headerElement) {
        $headerElement.append(
            '<a href="#" class="set-branch" title="The compiler repo bracnh from which the tests will be taken"><img src="' + this.setBranchIcon + '" /></a></span>' +
            ...
            .catch(function () {
                ...
                if (this.loadedSectionsNumber == 0) {
                    new Alert({
                        ...
                        content: "Tests for \"<b>" + sectionsPath.join(" → ") + " → " + sectionName + "</b>\" in <b>" + this.getBranch() + "</b> branch not yet written. <a href='https://github.com/JetBrains/kotlin/tree/master/compiler/tests-spec/testData' target='_blank'>Contribute it</a>!"
                    });
                }
    ...
    this.onSetBracnhIconClick = function () {
        var currentBracnh = window.localStorage.getItem("spec-tests-branch") || this.defaultBranch;
        var newBracnh = prompt("Specify the Kotlin compiler repo branch from which the spec tests will be taken:", currentBracnh);
        if (newBracnh && newBracnh != currentBracnh) {
            window.localStorage.setItem("spec-tests-branch", newBracnh);
        }
    }
