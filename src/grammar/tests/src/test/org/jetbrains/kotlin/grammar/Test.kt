...
class Test(private val filename: String) {
    companion object {
        private val antlrTreeFileHeaderPattern =
                Pattern.compile("""^File: .*?.kts?(?<isErrorExpected> \(ANTLR_PARSER_ERROR_EXPECTED\))?$""")
    ...
    fun doTest() {
        ...
        val dumpParseTree = parseTree.stringifyTree("File: ${testFile.name}" + (if (errorExpected) " (ANTLR_PARSER_ERROR_EXPECTED)" else ""))
